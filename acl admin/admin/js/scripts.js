...
//                        $this.find('span').html(start.format(drp.format) + drp.separator + end.format(drp.format));
    ...
    /*--------------------------------
        x-editable
     --------------------------------*/
    ULTRA_SETTINGS.xeditableExample = function() {

        if ($.isFunction($.fn.editable)) {
            
            $.fn.editable.defaults.mode = 'popup';     
                
                if($(".xedit-username").length){
                    //make username editable
                    $('.xedit-username').editable();
                }

                if($(".xedit-units").length){
                    //make unit editable
                    $('.xedit-units').editable();
                }               

                if($(".xedit-price").length){
                    //make price editable
                    $('.xedit-price').editable();
                }      

                if($(".xedit-status").length){
                    //make status editable
                    $('.xedit-status').editable({
                        type: 'select',
                        title: 'Select',
                        placement: 'right',
                        value: 2,
                ...
                if($(".xeditin-username").length){
                    //make username editable
                    $('.xeditin-username').editable();
                }

                if($(".xeditin-units").length){
                    //make unit editable
                    $('.xeditin-units').editable();
                }               

                if($(".xeditin-price").length){
                    //make price editable
                    $('.xeditin-price').editable();
                }      

                if($(".xeditin-status").length){
                    //make status editable
                    $('.xeditin-status').editable({
                        type: 'select',
                        title: 'Select',
                        placement: 'right',
                        value: 2,
        ...
        if ($.isFunction($.fn.iCheck)) {
            ...
            $('input.skin-square-orange').on('ifChecked', function (event){
                $(this).parent().parent().parent().addClass("rowchecked");          
            });
            $('input.skin-square-orange').on('ifUnchecked', function (event) {
                $(this).parent().parent().parent().removeClass("rowchecked");          
            });
    ...
    $(document).ready(function() {
        ...
        ULTRA_SETTINGS.xeditableExample();
