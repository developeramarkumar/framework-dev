...
class RegisterController extends Controller
{
	public function register(Request $request){
		...
		$message = 'Dear User Your www.makehappytrip.com Registration Verification OTP Is : '.$otp;
		if($userotp){
	...
	public function verifyOtp(Request $request){
		...
		// $used_referinput = Request::input('referid');
		// $referid = 'makemytrip'.substr(str_replace('.', null, str_replace(' ', null, microtime())), 1,7);
		$validate = $this->validate(Request::Input(),[
			'email' => 'required|email|unique:users',
			// 'name'=>'required',
			'otp' => [
		...
		// 	return array('status'=>500,'errors'=>['referid'=>'Refer id not match']);
		// }
		$user_insert = User::create(['email'=>Request::input('email'),'mobile'=>Request::input('mobile'),'password'=>Hash::make(Request::input('password')),'demo_password'=>\Crypt::encrypt(Request::input('password'))]);
		if($user_insert)
		{
			Auth::guard('user')->login($user_insert);
			UserOtp::where('mobile',$user_insert->mobile)->delete();
			$message = 'Congratulations Dear '.$user_insert->email.','.PHP_EOL.'Your Registration Has Been Completed. Please Visit makehappytrip.com and Login with Mobile No.:'.$user_insert->mobile.PHP_EOL.' and Password : '.\Crypt::decrypt($user_insert->demo_password);
